...
class ClipVAEBase(AnomalyDetectionBase):
    ...
    def detect(self, inputs):
        ...
        return self.kl_div_loss(mu, log_var)
    ...
    def loss_function(self, inputs, outputs, mu, log_var):
        ...
        kl_div_loss = self.kl_div_loss(mu, log_var)
        self.log_dict({
            "reco_loss": reco_loss.mean(),
            "kl_div_loss": kl_div_loss.mean(),
        })
        idxs = torch.argsort(reco_loss, dim=0)
        idxs = idxs[:int(len(idxs) * self.hparams["frac"])] 
        loss = reco_loss.mean() + self.hparams["beta"]*kl_div_loss[idxs].mean()
        return loss
